...
logger = Logger('BLiveClient').logger
...
class BLiveClient:
    ...
    async def join(self):
        ...
        if not self.is_running:
            logger.warning(
                'room=%s client is stopped, cannot join()', self.room_id)
            return
    ...
    async def _init_host_server(self):
        try:
            async with self._session.get(DANMAKU_SERVER_CONF_URL, params={'id': self._room_id, 'type': 0},
                ...
                if data['code'] != 0:
                    logger.warning(
                        'room=%d _init_host_server() failed, message=%s', self._room_id, data['message'])
                    return False
    ...
    def _parse_danmaku_server_conf(self, data):
        ...
        if not self._host_server_list:
            logger.warning(
                'room=%d _parse_danmaku_server_conf() failed: host_server_list is empty', self._room_id)
            return False
        return True
    ...
    async def _network_coroutine(self):
        ...
        while True:
            try:
                # 连接
                host_server = self._host_server_list[retry_count % len(
                    self._host_server_list)]
            ...
            # 准备重连
            retry_count += 1
            logger.warning('room=%d is reconnecting, retry_count=%d',
                           self.room_id, retry_count)
    ...
    async def _send_heartbeat(self):
        ...
        try:
            ...
            logger.warning(
                'room=%d _send_heartbeat() failed: %r', self.room_id, e)
    ...
    async def _on_ws_message(self, message: aiohttp.WSMessage):
        ...
        try:
            ...
            logger.exception(
                'room=%d _parse_ws_message() error:', self.room_id)
    ...
    async def _parse_ws_message(self, data: bytes):
        ...
        try:
            ...
            logger.exception(
                'room=%d parsing header failed, offset=%d, data=%s', self.room_id, offset, data)
            return
        ...
        if header.operation in (Operation.SEND_MSG_REPLY, Operation.AUTH_REPLY):
            ...
            body = data[offset + header.raw_header_size: offset +
                        header.raw_header_size + 4]
    ...
    async def _parse_business_message(self, header: HeaderTuple, body: bytes):
        ...
        if header.operation == Operation.SEND_MSG_REPLY:
            ...
            if body['code'] != AuthReplyCode.OK:
                raise AuthError(
                    f"auth reply error, code={body['code']}, body={body}")
            await self._websocket.send_bytes(self._make_packet({}, Operation.HEARTBEAT))
    ...
    async def _handle_command(self, command: dict):
        ...
        for res in results:
            if isinstance(res, Exception):
                logger.exception(
                    'room=%d _handle_command() failed, command=%s', self.room_id, command, exc_info=res)
